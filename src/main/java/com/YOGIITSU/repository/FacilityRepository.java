...
public interface FacilityRepository extends JpaRepository<BuildingFacility, Long> {
	...
	@Query(value = """
		    ...
		    WHERE bf.name = :name
		    ORDER BY b.name ASC, CAST(NULLIF(NULLIF(bf.floor, 'NULL'), '') AS SIGNED) ASC
		""", nativeQuery = true)
	List<FacilityLocationView> findLocationsByName(@Param("name") String name);

	// 단일 이름 + 특정 건물 - floor 문자열 정규화 추가
	@Query(value = """
		    SELECT b.name AS buildingName,
		    ...
		    ORDER BY CAST(NULLIF(NULLIF(bf.floor, 'NULL'), '') AS SIGNED) ASC
		""", nativeQuery = true)
	...
	// 대표 표기(IN) + 접두어 정규식(REGEXP) 동시 지원 (전체)
	@Query(value = """
		    SELECT b.name AS buildingName,
	...
	// 대표 표기(IN) + 접두어 정규식(REGEXP) 동시 지원 (특정 건물)
	@Query(value = """
		    SELECT b.name AS buildingName,
		           NULLIF(NULLIF(bf.floor, 'NULL'), '') AS floor   -- 문자열 'NULL' 제거
		    FROM building_facilities bf
