...
public class SignUpService {
	...
	public void register(MemberSignUpRequestDto dto) {
		...
		if (!Pattern.matches(userNamePattern, dto.getUserName())) {
			throw new InvalidArgumentException("이름은 2~8자의 영문, 한글, 숫자로만 입력해주세요. 특수문자는 사용할 수 없습니다.");
		}
	...
	private void validatePassword(String password) {
		if (password.length() < 8 || password.length() > 16) {
			throw new InvalidArgumentException("비밀번호는 8자 이상 16자 이하이어야 합니다.");
		}
		if (!Pattern.compile("[a-zA-Z]").matcher(password).find()) {
			throw new InvalidArgumentException("비밀번호에는 영문자가 최소 1자 이상 포함되어야 합니다.");
		}
		if (!Pattern.compile("[0-9]").matcher(password).find()) {
			throw new InvalidArgumentException("비밀번호에는 숫자가 최소 1자 이상 포함되어야 합니다.");
		}
		if (password.matches(".*[^a-zA-Z0-9*!_@#$].*")) {
			throw new InvalidArgumentException("비밀번호에는 *!_@#$ 외의 특수문자는 사용할 수 없습니다.");
		}
		if (!password.matches(".*[*!_@#$].*")) {
			throw new InvalidArgumentException("비밀번호에는 *!_@#$ 중 최소 1자 이상의 특수문자가 포함되어야 합니다.");
		}
	}

	private void validateMemberId(String memberId) {
		if (memberId.length() < 4 || memberId.length() > 20) {
			throw new InvalidArgumentException("아이디는 4자 이상 20자 이하이어야 합니다.");
		}
		if (!Pattern.compile("^[a-zA-Z]").matcher(memberId).find()) {
			throw new InvalidArgumentException("아이디는 반드시 영문자로 시작해야 합니다.");
		}
		if (!Pattern.compile("^[a-zA-Z0-9]+$").matcher(memberId).matches()) {
			throw new InvalidArgumentException("아이디는 영문자와 숫자로만 구성되어야 합니다.");
		}
	}
}